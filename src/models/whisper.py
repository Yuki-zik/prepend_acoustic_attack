...
class WhisperModel:
    ...
    '''
    def __init__(self, model_name='whisper-small', device=torch.device('cpu'), task='transcribe', language='en', dtype=torch.float32):
        # 将自定义名称映射到实际权重名称，并缓存到本地
        self.model_name = model_name
        self.model = whisper.load_model(MODEL_NAME_MAPPER[model_name], device=device, download_root=CACHE_DIR, dtype=dtype)
...
class WhisperModelEnsemble:
    ...
    '''
    def __init__(self, model_names=['whisper-small'], device=torch.device('cpu'), task='transcribe', language='en', dtype=torch.float32):
        # 加载多模型并共享 tokenizer，用于集成攻击或评估
        self.models = [whisper.load_model(MODEL_NAME_MAPPER[model_name], device=device, download_root=CACHE_DIR, dtype=dtype) for model_name in model_names]  # 逐个载入模型
